...
POIDS_ACTIVITES = {
    "sinistre": 1, "exp_lancee": 5, "exp_participee": 8,
...
bars1 = ax.bar(x - width/2, pct_effectifs, width, label="% des effectifs",        color=colors_list, alpha=0.6, edgecolor="black")
bars2 = ax.bar(x + width/2, pct_charges,   width, label="% de la charge ponderee", color=colors_list, alpha=1.0, edgecolor="black")
...
if log_membres:
    ...
    for i, role in enumerate(roles_track):
        ax.bar(x + i * width_attr,  ajouts[role],            width_attr, color=colors_attr[role], alpha=0.9, edgecolor="black", label=f"{role} (ajouts)")
        ax.bar(x + i * width_attr, [-s for s in suppr[role]], width_attr, color=colors_attr[role], alpha=0.4, edgecolor="black", label=f"{role} (departs)")
